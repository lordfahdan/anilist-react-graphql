...
const Detail = () => {
  ...
  const funcAddCollectionItem = async (data) => {
    let arrInput = [
      "<ul style='display:flex;align-items:flex-start;width:max-content;margin:auto;flex-direction: column;'>",
      '</ul>',
    ];
    let indexSplice = 0;
    if (state.length > 0) {
      state.forEach((e) => {
        if (getCollectionStatusById(e, data.id).notAdded) {
          ...
          arrInput.splice(
            indexSplice,
            0,
            `<li><label for="id${e.name}"><input id="id${e.name}" name="collections" type="checkbox" value="${e.name}"> ${e.name}</label></li>`
          );
        }
      });
      const { value: collections } = await Swal.fire({
        ...
        preConfirm: () => {
          ...
          const allInput = document.querySelectorAll(
            'input[name="collections"]:checked'
          );
      ...
      const { value: collections } = await Swal.fire({
        ...
        inputValidator: (result) => {
          ...
          if (result === '') return 'Collection Name cannot be empty';
          if (
            state.filter((e) => e.name.toLowerCase() === result.toLowerCase())
              .length > 0
          )
  ...
  const { loading, error, data } = useAnimeDetail({
    id
  });
      ...
      <CardDetailAnime>
        <DetailAnime>
          ...
          <div className="atributAnime">
            <div className="titleAnime">{Media.title.romaji}</div>
            ...
            <div className="descAnimeGenre">
              {Media.genres &&
                Media.genres.map((genre, index) => {
                  return (
                    <span key={`genre${index}`} className="spanGenre">
                      {genre}
                    </span>
                  );
                })}
        ...
        {state.length > 0 ? (
          getAllCollectionAddedStatus(state, Media.id).added ===
          state.length ? (
            <ButtonBookmark
              css={css`
                background: grey;
